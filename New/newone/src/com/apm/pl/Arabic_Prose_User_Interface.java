...
public class Arabic_Prose_User_Interface extends JFrame {
   ...
   private JPanel createBooksPanel() {
    ...
    private void openChaptersPanel() {
     ...
     // ===== Header Panel with Book Icon + Book Name on Right =====
        ...
        titleContent.add(titleLabel);  // this is pasting the book title and book pic on header
        ...
        if (chapters != null && !chapters.isEmpty()) {
            for (ChapterDTO chapter : chapters) {
                chapterListModel.addElement(chapter.getChapterName()); // Removed bullet points
            }
        } else {
    ...
    private void openSentencesPanel(JDialog parentDialog) {
     ...
     // ===== Header with Chapter Icon + Title on Left =====
        JPanel titlePanel = new JPanel(new BorderLayout()) {
            @Override
            protected void paintComponent(Graphics g) {
                super.paintComponent(g);
        ...
        titlePanel.setOpaque(false);
        titlePanel.setBorder(new EmptyBorder(12, 15, 12, 15)); // slightly taller header

        JLabel chapterIconLabel;
        try {
            ImageIcon chapterIcon = new ImageIcon("bin/icons/chapter.png");
            Image scaledIcon = chapterIcon.getImage().getScaledInstance(28, 28, Image.SCALE_SMOOTH);
            chapterIcon = new ImageIcon(scaledIcon);
            chapterIconLabel = new JLabel(chapterIcon);
        } catch (Exception e) {
            System.out.println("❌ Could not load chapter icon: " + e.getMessage());
        ...
        // Combine icon and title in a panel aligned to left
 ...
 // ==================== ✍️ AUTHORS PANEL ====================
    ...
    private void oneTapAddBook() {
        ...
        JLabel titleLabel;
        try {
            ImageIcon rocketIcon = new ImageIcon("bin/icons/rocket.png"); // You can add a rocket icon
            Image scaledRocket = rocketIcon.getImage().getScaledInstance(48, 48, Image.SCALE_SMOOTH);
            titleLabel = new JLabel("  One-Tap Book Import", new ImageIcon(scaledRocket), JLabel.LEFT);
        } catch (Exception e) {
            titleLabel = new JLabel("  🚀 One-Tap Book Import");
        }
        ...
        JLabel subtitle = new JLabel("Upload a text file to automatically extract chapters and sentences");
        ...
        // ===== FILE SELECTION AREA =====
        JPanel filePanel = createFileSelectionArea(oneTapDialog);
    ...
    /**
     * 📁 Create animated file selection area
     */
    private JPanel createFileSelectionArea(JDialog parentDialog) {
        ...
        JLabel formatText = new JLabel("Supports: .txt files");
                ...
                try {
                    ...
                    if (files.size() > 0) {
                        ...
                        if (file.getName().toLowerCase().endsWith(".txt")) {
                            processSelectedFile(file, parentDialog);
                        } else {
    ...
    private JPanel createProgressIndicator() {
        ...
        JLabel progressLabel = new JLabel("Processing your book...");
     ...
     */
    private void openFileChooser(JDialog parentDialog) {
        ...
        if (result == JFileChooser.APPROVE_OPTION) {
            ...
            processSelectedFile(selectedFile, parentDialog);
        }
    }

    /**
     * 🔄 Process selected file and extract content
     */
    private void processSelectedFile(java.io.File file, JDialog parentDialog) {
        // Show progress
        ...
        // Simulate processing with timer (replace with actual processing)
        Timer processTimer = new Timer(2000, new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                try {
                    // Read file content
                    ...
                    // Extract book info and create book
                    String fileName = file.getName().replace(".txt", "");
                    boolean success = extractAndCreateBook(content, fileName);
                    
                    progressPanel.setVisible(false);
                    
                    if (success) {
                        showSuccessMessage("Book '" + fileName + "' imported successfully!\n" +
                                         "Chapters and sentences extracted automatically.");
                        parentDialog.dispose();
                        loadBooks(); // Refresh book list
                    } else {
                        showErrorMessage("Failed to import book. Please try again.");
                    }
                } catch (Exception ex) {
                    progressPanel.setVisible(false);
                    showErrorMessage("Error reading file: " + ex.getMessage());
                    ex.printStackTrace();
                }
            }
        });
    ...
    /**
     * 📖 Extract content and create book with chapters/sentences
     */
    private boolean extractAndCreateBook(String content, String bookTitle) {
        try {
            // Get first available author or create a default one
            ArrayList<AuthorDTO> authors = BussinessLayerFasade.getAllAuthors();
            String authorName;
            if (authors != null && !authors.isEmpty()) {
                ...
                BussinessLayerFasade.createAuthor(authorName, "Automatically created author for imported books");
            }
            
            // Create book
            boolean bookCreated = BussinessLayerFasade.createBook(bookTitle, authorName, "Modern");
            if (!bookCreated) {
                return false;
            }
            
            // Extract chapters based on common chapter markers
            String[] chapterSplitters = {"\nChapter", "\nCHAPTER", "\nالفصل", "\n\\d+\\."};
            String[] chapters = content.split(String.join("|", chapterSplitters));
            
            if (chapters.length == 0) {
                chapters = new String[]{content}; // Treat entire content as one chapter
            }
            
            // Create chapters and sentences
            for (int i = 0; i < chapters.length; i++) {
                String chapterContent = chapters[i].trim();
                ...
                // Create chapter
                boolean chapterCreated = BussinessLayerFasade.createChapter(bookTitle, chapterName);
                if (chapterCreated) {
                    // Extract sentences using multiple delimiters
                    String[] sentences = chapterContent.split("[.!?]+");
                    
                    for (String sentence : sentences) {
                        sentence = sentence.trim();
                        if (sentence.length() > 5) { // Only add meaningful sentences
                            BussinessLayerFasade.createSentence(chapterName, sentence, null, null, null);
                        }
                    }
                }
            }
