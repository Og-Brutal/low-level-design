...
public class App {
	 ...
	 public static void main(String[] args) throws ClassNotFoundException, InstantiationException, IllegalAccessException, UnsupportedLookAndFeelException {
		...
		try {
			daf = new DataAccessLayerFasade(bookDAO,new AuthorDAO(conn,conn.createStatement()),new SentenceDAO(conn,conn.createStatement()),new ChapterDAO(conn,conn.createStatement()),new TextExtracter(),new TokenDAO(conn,conn.createStatement()));
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		  BookBO bookBO = new BookBO (daf);
		  BussinessLayerFasade blf = new BussinessLayerFasade(bookBO,new AuthorBO(daf),new SentenceBO(daf),new ChapterBO(daf),new TokenBO(daf));
	        try {
