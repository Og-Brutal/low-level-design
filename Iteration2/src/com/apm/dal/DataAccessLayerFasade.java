...
public class DataAccessLayerFasade implements IDataAccessLayerFasade{
	...
	//title , author id ,era
	public DataAccessLayerFasade(IBookDAO book,IAuthorDAO author,ISentenceDAO sentence,IChapterDAO chapter,ITextExtracter textExtracter,ITokenDAO token)
	{
		this.book = book;
		this.author = author;
	...
	public boolean addToken(int sentenceID, String text) {
		return token.addToken(sentenceID, text);
	}
	@Override
	public int searchToken(String text) {
		return token.searchToken(text);
	}
	@Override
	public TokenDTO getToken(String text) {
		return token.getToken(text);
	}
	@Override
